  ...
  return (
    <div className='bg-zinc-950 md:flex'>
      <div className='font-bold md:text-[50px] text-[35px] md:p-[150px] p-[50px] md:pl-[250px] pl-[50px] text-white font-serif'>
        {title}
        <div className='mb-5 text-slate-800 text-[19px] md:w-[700px] w-[300px] '>{discription}</div>
        <div className='md:grid grid-cols-2 gap-6 md:gap-12 mt-6'>
          {services.map((service, index) => (
            <div key={index} className='md:flex items-center md:gap-2'>
              <IoMdCheckboxOutline md:size={30} size={25} className='text-red-700' />
              <div className=' md:mt-0 mt-[-25px] md:ml-0 ml-[50px] md:text-[20px] text-[16px] text-slate-800'>{service}</div>
            </div>
          ))}
        </div>
